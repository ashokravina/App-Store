...
class AppStore extends Component {
  state = {searchInput: '', addTabItem: tabsList[0].tabId}
  ...
  getchangeTabButton = () => {
    ...
    const filterdTabItem = appsList.filter(each => each.category === addTabItem)
    return filterdTabItem
  }
  ...
  getSearchAppItem = () => {
    const {searchInput, addTabItem} = this.state
    const filterdAppItem = appsList.filter(
      each =>
        each.appName.toLowerCase().includes(searchInput.toLowerCase()) &&
        each.category === addTabItem,
    )
    return filterdAppItem
  }
  render() {
    const {searchInput, addTabItem} = this.state
    const filterdTabItem = this.getchangeTabButton()
    const filterdAppItem = this.getSearchAppItem()
    return (
        ...
        <ul className="tab-container">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              isActive={addTabItem === eachTab.tabId}
            />
          ))}
        </ul>
        <div>
          <ul className="apps-container">
            {filterdAppItem.map(eachApp => (
